...
public class MyCreateTableActivity extends AppCompatActivity {
    ...
    private ListView lv_createTables;
    private MyRTableAdapter myRTableAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState){
        super.onCreate(savedInstanceState);
        setContentView(R.layout.mine_createtables);

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        toolbar.setNavigationIcon(R.drawable.ic_keyboard_backspace_black_24dp);
        toolbar.setTitle("我创建的圆桌");
        ...
        lv_createTables=findViewById(R.id.mine_createtables);
        myRTableAdapter=new MyRTableAdapter();
        myRTableAdapter.setContext(this);
        List<RTableSimple> rtables=getMyCreateTables();
        if(rtables.size()==0){
            Toast.makeText(getApplicationContext(),"您当前还没有创建圆桌~",Toast.LENGTH_SHORT).show();
        }else{
            myRTableAdapter.setmData(rtables);

            lv_createTables.setAdapter(myRTableAdapter);
            lv_createTables.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
    ...
    public List<RTableSimple> getMyCreateTables(){
        try {
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try{
                        String result = WebService.getUserCreateAll();
    ...
    private void transIntentChatting(View v,String tid,String tname,String tphoto){
        Intent intent=new Intent(MyCreateTableActivity.this, ChatActivity.class);
